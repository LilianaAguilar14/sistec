...
export default function TicketsTecnico({ usuario }: { usuario: any }) {
  ...
  useEffect(() => {
    fetch("https://localhost:7232/api/Ticket")
      .then((res) => res.json())
      .then((data) => {
        const asignados = data.filter((t: any) => t.idUsuarioTecnico === usuario.id)
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold">Mis Tickets Asignados</h2>
      <Card>
              ...
              <TableBody>
                {tickets.map((ticket) => (
                  <TableRow key={ticket.idTicket}>
                    ...
                    <TableCell>
                      <Badge className={getColor(ticket.estado)}>{ticket.estado}</Badge>
                    </TableCell>
